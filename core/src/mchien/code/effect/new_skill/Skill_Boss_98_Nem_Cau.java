...
import mchien.code.model.Boss_98;
import mchien.code.model.EffectManager;
import lib.mVector;

public class Skill_Boss_98_Nem_Cau extends Me_Arrow {
   public Boss_98 boss;
   public mVector allTarget = new mVector();

   public Skill_Boss_98_Nem_Cau() {
      this.xw = new int[1];
      this.yw = new int[1];
   }

   public Skill_Boss_98_Nem_Cau(short x, short y, Actor target, byte type, byte effect, byte lathinh, byte Position, int yadd) {
      ...
      if (this.boss != null) {
         ...
         if (this.boss.isCreateWeapon) {
            ...
            for(int i = 0; i < size; ++i) {
               ...
               Skill_DefaultBoss_98 e = new Skill_DefaultBoss_98(this.boss.x, (short)(this.boss.y - this.boss.getHeight()), ac, (byte)119, (byte)45, (byte)-2, (byte)32, -ac.getHeight() * 2 / 3);
