...
import mchien.code.screen.screen.ObjGame;
import mchien.code.screen.screen.ScreenTeam;
import javax.microedition.lcdui.Image;
import lib.mGraphics;
import mchien.plugins.InputKeyBoard;
...
public class TField extends ObjGame implements IActionListener {
   ...
   private void keyPressedAscii(int keyCode) {
      if (this.inputType != 2 && this.inputType != 3 || keyCode >= 48 && keyCode <= 57 || keyCode >= 65 && keyCode <= 90 || keyCode >= 97 && keyCode <= 122) {
         if (this.text.length() < this.maxTextLenght) {
            String textNew = (char)keyCode + "";
            if(keyCode == 118 && GameCanvas.isCtrl)
               textNew = Gdx.app.getClipboard().getContents();
            String ttext = this.text.substring(0, this.caretPos) + textNew;
            if (this.caretPos < this.text.length()) {
               ttext = ttext + this.text.substring(this.caretPos, this.text.length());
            }

            this.text = ttext;
            this.caretPos += textNew.length();
            this.setPasswordTest();
            this.setOffset();
         }
   ...
   public void setFocus(boolean isfocus) {
      ...
      if (!this.ischat && GameCanvas.gameScr != null && (GameCanvas.gameScr.chatMode || GameCanvas.gameScr.chatWorld)) {
         this.ischat = true;
      }

      if(this.isFocus &&
              paintFocus && Gdx.app.getType() != Application.ApplicationType.Desktop &&
              (this.title != "" || this.strnull != "") &&
              (input == null || !input.isShow))
      {
         input = new InputKeyBoard(this);
         input.showKeyboard((this.title == ""?this.strnull:this.title), this.text);
      }
   }
