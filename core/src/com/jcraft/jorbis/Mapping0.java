...
class Mapping0 extends FuncMapping {
   ...
   synchronized int inverse(Block vb, Object l) {
      ...
      for(i = 0; i < vi.channels; ++i) {
         pcm = vb.pcm[i];
         j = info.chmuxlist[i];
         this.floormemo[i] = look.floor_func[j].inverse1(vb, look.floor_look[j], this.floormemo[i]);
      ...
      for(i = info.coupling_steps - 1; i >= 0; --i) {
         pcm = vb.pcm[info.coupling_mag[i]];
      ...
      for(i = 0; i < vi.channels; ++i) {
         pcm = vb.pcm[i];
         j = info.chmuxlist[i];
         look.floor_func[j].inverse2(vb, look.floor_look[j], this.floormemo[i], pcm);
      }
