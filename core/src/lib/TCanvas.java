...
public abstract class TCanvas implements Runnable {
   ...
   private void checkZoomLevel(int w, int h) {
      if (!Main.isJava) {
         ...
         } else if (TCanvas.ScreenSize ==1) {
            mGraphics.zoomLevel = 1;
         } else if (Gdx.graphics.getWidth() * Gdx.graphics.getHeight() > 2073600) {
            mGraphics.zoomLevel = 4;
         } else if (Gdx.graphics.getWidth() * Gdx.graphics.getHeight() > 691200) {
            mGraphics.zoomLevel = 3;
         } else if (Gdx.graphics.getWidth() * Gdx.graphics.getHeight() > 135600) {
