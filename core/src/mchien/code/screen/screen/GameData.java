...
public class GameData {
   ...
   public static void removeAllImgTree() {
      Enumeration k = listImgIcon.keys();
      ...
      while(true) {
         ...
         do {
            if (!k.hasMoreElements()) {
               for(int i = 0; i < keyremove.size(); ++i) {
                  key = (String)keyremove.elementAt(i);
         ...
         } while((intKey < Res.ID_ITEM_MAP || intKey >= Res.ID_START_SKILL) && intKey >= Res.ID_ITEM && (intKey < Res.ID_START_SKILL || intKey >= Res.ID_ICON_NPC));
   ...
   public static ImageIcon getImgIcon(short id) {
      ...
      if (img != null && (img == null || img.img != null)) {
         ...
         if (img.img == null && mSystem.currentTimeMillis() - img.timeGetBack >= 0L) {
            ...
            if(id >= Res.ID_IMAGE_BAUCUA);
            else if(id >= Res.ID_IMAGE_GOINAP);
            else if (id >= Res.ID_EFFECT_SKILL) {
