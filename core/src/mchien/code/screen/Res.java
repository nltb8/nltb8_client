...
import mchien.code.model.CharPartInfo;
import mchien.code.model.Chunk;
import mchien.code.model.ChunkData;
import mchien.code.model.ChunkDemo;
import mchien.code.model.ChunkTemplate;
import mchien.code.model.FrameImage;
...
import mchien.code.model.WPSplashInfo;
import mchien.code.model.mCommand;
import mchien.code.network.GameService;
import mchien.code.screen.screen.CreateCharScr;
...
public class Res {
   ...
   public static final int ID_IMAGE_GOINAP = 29_000;
   public static final int ID_IMAGE_BAUCUA = 30_000;
   ...
   public static void loadChunkPrivate(byte[] data, int chunkID) {
      if (chunkID != -1) {
         ...
         try {
            ...
            for(j = 0; j < chunk.chunkData.length; ++j) {
      ...
      if (chunkID != -1) {
         if (!loadChunkPrivateRms(chunkID)) {
            ...
            try {
               ...
               if (mSystem.isj2me) {
                  DataGetChunkChar data = (DataGetChunkChar)listGetChunk.get(String.valueOf(chunkID));
                  if (data == null) {
                     listGetChunk.put(String.valueOf(chunkID), new DataGetChunkChar());
   ...
   public static int[] trimArray(int[] array) {
      ...
      for(i = 0; i < array.length; ++i) {
         if (temp.size() == 0) {
            temp.addElement(String.valueOf(array[i]));
         } else {
            boolean dup = false;
