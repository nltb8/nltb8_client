...
class Floor1 extends FuncFloor {
   ...
   Object look(DspState vd, InfoMode mi, Object i) {
      ...
      int foo;
      for(foo = 0; foo < info.partitions; ++foo) {
         _n += info.class_dim[info.partitionclass[foo]];
      }

      _n += 2;
      look.posts = _n;

      for(foo = 0; foo < _n; sortpointer[foo] = foo++) {
      }
      ...
      for(j = 0; j < _n - 1; ++j) {
         for(lo = j; lo < _n; ++lo) {
            if (info.postlist[sortpointer[j]] > info.postlist[sortpointer[lo]]) {
               foo = sortpointer[lo];
               sortpointer[lo] = sortpointer[j];
               sortpointer[j] = foo;
            }
         }
      }
      ...
      for(j = 0; j < _n; look.reverse_index[look.forward_index[j]] = j++) {
      }
   ...
   Object inverse1(Block vb, Object ii, Object memo) {
      ...
      if (vb.opb.read(1) == 1) {
         ...
         int room;
         int val;
         for(predicted = 2; i < info.partitions; ++i) {
            hiroom = info.partitionclass[i];
            int cdim = info.class_dim[hiroom];
            room = info.class_subs[hiroom];
            val = 1 << room;
            int cval = 0;
            if (room != 0) {
               cval = books[info.class_book[hiroom]].decode(vb.opb);
               if (cval == -1) {
                  return null;
               }
            }
            ...
            for(int k = 0; k < cdim; ++k) {
               int book = info.class_subbook[hiroom][cval & val - 1];
               cval >>>= room;
               if (book >= 0) {
                  if ((fit_value[predicted + k] = books[book].decode(vb.opb)) == -1) {
                     return null;
                  }
               } else {
                  fit_value[predicted + k] = 0;
               }
            }
         ...
         for(i = 2; i < look.posts; ++i) {
            predicted = render_point(info.postlist[look.loneighbor[i - 2]], info.postlist[look.hineighbor[i - 2]], fit_value[look.loneighbor[i - 2]], fit_value[look.hineighbor[i - 2]], info.postlist[i]);
            hiroom = look.quant_q - predicted;
            room = (hiroom < predicted ? hiroom : predicted) << 1;
            val = fit_value[i];
            if (val != 0) {
               if (val >= room) {
                  if (hiroom > predicted) {
               ...
               int var10001 = look.loneighbor[i - 2];
               fit_value[var10001] &= 32767;
               var10001 = look.hineighbor[i - 2];
               fit_value[var10001] &= 32767;
            } else {
