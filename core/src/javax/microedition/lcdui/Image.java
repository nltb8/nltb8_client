...
//            System.out.println("Load Image: "+LibSysTem.res + url);
         }
   ...
   public int getWidth() {
      return this.width / mGraphics.zoomLevel;
//      return this.width / 2;
   }
...
//      return this.height / 2;
   ...
   public void setShaderImage(){
      // 1. Lấy Pixmap từ Texture
      Pixmap originalPixmap = textureToPixmap(texture);

// 2. Tạo Pixmap mới (hoặc sửa trực tiếp trên cái cũ cũng được)
      Pixmap grayscalePixmap = new Pixmap(originalPixmap.getWidth(), originalPixmap.getHeight(), originalPixmap.getFormat());

// 3. Duyệt pixel và chuyển grayscale
      for (int y = 0; y < originalPixmap.getHeight(); y++) {
         for (int x = 0; x < originalPixmap.getWidth(); x++) {
            ...
            grayscalePixmap.drawPixel(x, y, newPixel);
         }
      }

// 4. Tạo Texture mới
      Texture grayscaleTexture = new Texture(grayscalePixmap);

// Đừng quên dispose Pixmap cũ nếu không cần nữa
      originalPixmap.dispose();

      this.texture = grayscaleTexture;
   }
