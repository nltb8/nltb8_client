...
import mchien.code.model.ChatDetail;
import mchien.code.model.IActionListener;
import mchien.code.model.ListNew;
import mchien.code.model.MainTextChat;
import mchien.code.model.Scroll;
import mchien.code.model.T;
import mchien.code.model.mCamera;
...
public class MsgChat extends ScreenTeam implements IActionListener {
   ...
   public void paint(mGraphics g) {
      ...
      paintDialogNew(g, this.xDia - 10, this.yDia - GameCanvas.hCommand - 6, this.wDia + 20, this.hDia + GameCanvas.hCommand + 12, 0);
      int xp = this.xDia + this.xdich;
      int yp = this.yDia + this.wOne5;
      if (vecChatTab.size() > 0 && curentfocus != null) {
         if (this.min > 0) {
            g.drawRegion(ScreenTeam.imgTab[13], 0, this.isLeft && GameCanvas.gameTick % 6 < 3 ? 16 : 0, 13, 8, 6, xp - 6, yp + this.wOne5 * 2, mGraphics.VCENTER | mGraphics.HCENTER, true);
         }
         ...
         for(i = this.min; i < idSelect; ++i) {
            chat = (ChatDetail)vecChatTab.elementAt(i);
            this.paintRectNew_(g, xp, yp, this.wOneItem, this.wOneItem, chat.isNew, chat.timeNew);
            mFont.tahoma_7b_white.drawString(g, strName[i], xp + this.wOneItem / 2 + 2, yp + this.wOneItem / 2 - 7, 2, true);
            xp += this.wOneItem + 5;
         }
         ...
         for(i = this.max - 1; i >= idSelect + 1; --i) {
            chat = (ChatDetail)vecChatTab.elementAt(i);
            this.paintRectNew(g, xp + (i - idSelect - 1) * (this.wOneItem + 4), yp, this.wOneItem, this.wOneItem, chat.isNew, chat.timeNew);
            mFont.tahoma_7b_white.drawString(g, strName[i], xp + this.wOneItem / 2, yp + this.wOneItem / 2 - 7, 2, true);
         }

         if (this.max < vecChatTab.size()) {
            g.drawRegion(ScreenTeam.imgTab[13], 0, this.isRight && GameCanvas.gameTick % 6 < 3 ? 24 : 8, 13, 8, 6, xp + 7, yp + this.wOne5 * 2, mGraphics.VCENTER | mGraphics.HCENTER, true);
         }

         this.paintTabFocus(g, this.xselect, this.yDia - GameCanvas.hCommand - 6 + 13 + GameCanvas.hCommand - 20, this.wMainTab);
         String strname = "";
         if (mFont.tahoma_7b_white.getWidth(curentfocus.name) < this.wMainTab - 4) {
            strname = curentfocus.name;
         } else {
            strname = curentfocus.name.substring(0, (this.maxText - 1) * 2) + "..";
         }

         mFont f = mFont.tahoma_7b_white;
         f.drawString(g, strname, this.xselect + this.wMainTab / 2, yp + this.wOneItem / 2 - 7, 2, true);
         yp += this.wOneItem;
         ...
         for(i = this.minchat; i <= this.maxchat; ++i) {
            if (i < curentfocus.vecDetail.size() && i >= 0) {
               MainTextChat t = (MainTextChat)curentfocus.vecDetail.elementAt(i);
               if (t.color == 1) {
                  mFont.tahoma_7_green.drawString(g, t.text, this.xDia + this.wOne5 + this.wDia, yp + 2 + i * GameCanvas.hText, 1, true);
               } else {
                  mFont.tahoma_7_white.drawString(g, t.text, this.xDia + this.wOne5, yp + 2 + i * GameCanvas.hText, 0, true);
               }
            }
         }
