...
public class InfoOtherCharScr extends ScreenTeam implements IActionListener {
   ...
   public void updateTouch() {
      if (GameCanvas.currentDialog == null) {
         ...
         if(selected == -1|| charFocus.equip[selected]== null){
            if(isPaint)
               return;
            isPaint = true;
            int x01 = this.x + this.size / 2 + selected % this.colum * this.size + 16;
            int y00 = 11 + this.y + selected / this.colum * this.size;
            mVector text = new mVector();
            for(int i = 0; i < options.size(); ++i) {
               ItemOption itop = (ItemOption)options.elementAt(i);
               mVector m = itop.getInfoTextShow(0);
               for(int j = 0; j < m.size(); ++j) {
                  text.addElement(m.elementAt(j));
               }
            }
            this.setShowText(text, x01, y00, (String[])null, true);
         }else
            isPaint = false;
   ...
   private void setShowText(mVector text, int x0, int y0, String[] arr, boolean isTile) {
      if (text != null || arr != null) {
         ...
         if (selected > -1 && selected < charFocus.equip.length && charFocus.equip[selected + this.indexWearing] != null) {
            ...
            this.numKhamNgoc = charFocus.equip[selected + this.indexWearing].numKhamNgoc;
            this.idNgocKham = charFocus.equip[selected + this.indexWearing].idNgocKham;
