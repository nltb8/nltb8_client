...
public class Scroll {
    ...
    private ScrollResult updateKeyScrollUpDown() {
        ...
        if (GameCanvas.isPointerDown[0]) {
            if (!this.pointerIsDowning && GameCanvas.isPointerNj(xTT, yTT, wTT, hTT, 0)) {
                ...
                if ((dx = GameCanvas.py[0] - this.pointerDownLastX[0]) != 0 && this.selectedItem != -1) {
                    this.selectedItem = -1;
                }
    ...
    private ScrollResult updateKeyScrollLeftRight() {
        ...
        if (GameCanvas.isPointerDown[0]) {
            if (!this.pointerIsDowning && GameCanvas.isPointerNj(xTT, yTT, wTT, hTT, 0)) {
                ...
                if ((dx = GameCanvas.px[0] - this.pointerDownLastX[0]) != 0 && this.selectedItem != -1) {
                    this.selectedItem = -1;
                }
                int i = this.pointerDownLastX.length - 1;
                while (i > 0) {
    ...
    public void moveTo(int to) {
        if (this.styleUPDOWN) {
            this.cmtoY = to -= (this.height - this.ITEM_SIZE * 2) / 2;
            if (this.cmtoY < 0) {
                this.cmtoY = 0;
            }
            if (this.cmtoY > this.cmyLim) {
            ...
            this.cmtoX = to -= (this.width - this.ITEM_SIZE) / 2;
            if (this.cmtoX < 0) {
                this.cmtoX = 0;
            }
            if (this.cmtoX > this.cmxLim) {
