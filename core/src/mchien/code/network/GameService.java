...
import mchien.code.model.ReadMessenge;
...
//            m.writer().writeByte(2);
     ...
     * Tạo phòng Bầu Cua
     */
    public void bauCuaCreateRoom() {
        Message m = this.init(BAUCUA_CREATE_ROOM);
        this.session.sendMessage(m);
        m.cleanup();
    }
     ...
     */
    public void bauCuaJoinRoom(int roomId) {
        Message m = this.init(BAUCUA_JOIN_ROOM);
        try {
            m.writer().writeInt(roomId);
            this.session.sendMessage(m);
            m.cleanup();
        } catch (IOException e) {
            e.printStackTrace();
     ...
     * Rời phòng Bầu Cua
     */
    public void bauCuaLeaveRoom() {
        Message m = this.init(BAUCUA_LEAVE_ROOM);
        this.session.sendMessage(m);
        m.cleanup();
    }
     ...
     * Sẵn sàng chơi
     */
    public void bauCuaReady(boolean isReady) {
        Message m = this.init(BAUCUA_READY);
        try {
    ...
    /**
     * Đặt cược
     * @param animalIndex 0-5 (Bầu, Cua, Tôm, Cá, Nai, Gà)
     * @param betAmount số tiền cược
     */
    public void bauCuaPlaceBet(int animalIndex, long betAmount) {
        Message m = this.init(BAUCUA_PLACE_BET);
        try {
            m.writer().writeByte(animalIndex);
            m.writer().writeLong(betAmount);
            this.session.sendMessage(m);
            m.cleanup();
        } catch (IOException e) {
            e.printStackTrace();
     ...
     */
    public void bauCuaChat(String name,String message) {
        Message m = this.init(BAUCUA_CHAT);
        try {
            m.writer().writeUTF(name);
     ...
     * @param playerId ID của player cần đuổi
     */
    public void bauCuaKickPlayer(int playerId) {
        Message m = this.init(BAUCUA_KICK_PLAYER);
        try {
            m.writer().writeInt(playerId);
            this.session.sendMessage(m);
            m.cleanup();
        } catch (IOException e) {
            e.printStackTrace();
     ...
     */
    public void bauCuaSetRoomPassword(String password) {
        Message m = this.init(BAUCUA_SET_PASSWORD);
        try {
    ...
    /**
     * Đặt tiền cược mặc định
     * @param amount số tiền cược mặc định
     */
    public void bauCuaSetDefaultBet(long amount) {
        Message m = this.init(BAUCUA_SET_DEFAULT_BET);
        try {
    ...
    /**
     * Gửi lệnh bắt đầu game (chỉ chủ phòng)
     */
    public void bauCuaStartGame() {
        Message m = this.init(BAUCUA_START_GAME);
        try {
            this.session.sendMessage(m);
            m.cleanup();
        } catch (Exception e) {
    ...
    // Removed bauCuaRequestDiceResult() - Server tự động gửi kết quả
...
//         }
    ...
    public void removeEquipt(short id) {
        Message msg = this.init((byte) -76);
    ...
    public void TradeProces(int type) {
        TradeProces(type, null, -1, null);
    }

    public void TradeProces(int type, String name) {
        TradeProces(type, name, -1, null);
    }

    public void TradeProces(int type, int xu, mVector itemSend) {
        TradeProces(type, null, xu, itemSend);
    }

    public void TradeProces(int type, String namePlayer, int xu, mVector itemSend) {
        try {
            Message msg = this.init((byte) -77);

            msg.writer().writeByte(type);
            if (type == 0) {
                msg.writer().writeInt(xu);
                msg.writer().writeShort(itemSend.size());
                for (int i = 0; i < itemSend.size(); i++) {
                    Item it = (Item) itemSend.elementAt(i);
                    msg.writer().writeShort(it.ID);
                    msg.writer().writeInt(it.quantity);
                }
            } else if (type == -1)
                msg.writer().writeUTF(namePlayer);
            this.session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
    ...
    public void buyGoiNap(int idGoiNap, int idBox) {
        try {
            Message msg = this.init((byte) -78);

            msg.writer().writeByte(1);
            msg.writer().writeShort(idGoiNap);
            msg.writer().writeByte(idBox);
    ...
    public void changeTabGoiNap(int idTab) {
        try {
            Message msg = this.init((byte) -78);

            msg.writer().writeByte(0);
            msg.writer().writeByte(idTab);
    ...
    public void inputGiftCode(String s) {
        try {
            Message msg = this.init((byte) -78);
    ...
    public void doOpenUIMount(){
        Message msg = this.init((byte) -80);
        try {
            this.session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception var3) {
    ...
    public void doMountEat(short iditem, byte type) {
        Message msg = this.init((byte) -82);
    ...
    public void removeMountEquip(byte id) {
        Message msg = this.init((byte) -81);
