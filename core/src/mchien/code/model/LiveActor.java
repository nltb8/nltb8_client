...
public class LiveActor extends Actor {
   ...
   public void paintInfoFocus(mGraphics g) {
      if (this.timePaintinfo - mSystem.currentTimeMillis() > 0L) {
         ...
         g.fillRect(this.x - w / 2 + dx, this.y - this.getHeight() + 10 - this.getYfly(), w, 3, false);
         int wpPaint = 0;
         long tile = (long)w;
         long objFocushp = (long)this.hp;
         long maxHp = tile * objFocushp;
         wpPaint = (int)(maxHp / (long)this.maxhp);
         if (wpPaint <= 0) {
            wpPaint = 1;
         } else if (wpPaint > w) {
            wpPaint = w;
         ...
         g.setColor(color);
         g.fillRect(this.x - w / 2 + 1 + dx, this.y - this.height + 10 + 1 - this.getYfly(), wpPaint - 2, 1, false);
         mFont.tahoma_7_yellow.drawString(g, this.name, this.x, this.y + 10 - (this.height + 15) - this.getYfly(), 3, false);
      }
   ...
   public void jumpVang(Actor causeBy) {
      if (causeBy != null) {
         ...
         for(this.dvangy = (short)((this.y - causeBy.y) * 20); this.dvangx > 4 || this.dvangy > 4 || this.dvangx < -4 || this.dvangy < -4; this.dvangy >>= 1) {
            this.dvangx >>= 1;
         }
   ...
   public void paintTopDataEff(mGraphics g, int dy) {
      int i;
      DataSkillEff dataeff;
      for(i = 0; i < this.DataEff.size(); ++i) {
         dataeff = (DataSkillEff)this.DataEff.elementAt(i);
         if (dataeff != null) {
      ...
      for(i = 0; i < this.vecEFfect_Skill.size(); ++i) {
         dataeff = (DataSkillEff)this.vecEFfect_Skill.elementAt(i);
         if (dataeff != null) {
            int h = 0;
            if (this.isMonster()) {
               String pos = (String)GameScr.hashEffInfo.get(String.valueOf(dataeff.idEff));
               if (pos != null) {
                  int check = Integer.parseInt(pos);
                  if (check == 1) {
                     h = 55 - this.getHeight();
                  } else if (check == 2) {
                     h = 27 - this.getHeight() / 2;
                  } else if (check > 0) {
                     h = 27 - this.getHeight() / 2 - check;
                  }
               }
            }
   ...
   public boolean mcanmove() {
      for(int i = 0; i < this.vecEFfect_Skill.size(); ++i) {
         DataSkillEff data = (DataSkillEff)this.vecEFfect_Skill.elementAt(i);
         if (data != null && data.canActorMove) {
            return false;
         }
      }
   ...
   public boolean canPaint() {
      for(int i = 0; i < this.vecEFfect_Skill.size(); ++i) {
         DataSkillEff data = (DataSkillEff)this.vecEFfect_Skill.elementAt(i);
         if (data != null && data.canPaintActor) {
            return false;
         }
      }
   ...
   public boolean canUseSkill() {
      for(int i = 0; i < this.vecEFfect_Skill.size(); ++i) {
         DataSkillEff data = (DataSkillEff)this.vecEFfect_Skill.elementAt(i);
         if (data != null && data.canActorFight) {
            return false;
         }
      }
