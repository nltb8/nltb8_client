...
public abstract class Dropable extends Actor {
   ...
   public void update() {
      ...
      if (this.catagory != 3 && this.catagory != 6) {
         if (this.catagory == 4) {
            int time = 15000;
            time = (char)(this.item_template_id >= 10 ? '\uea60' : time);
            if (now - this.timeStart > (long)time) {
               this.wantDestroy = true;
            }
         }
      } else if (now - this.timeStart > 25000L) {
         this.wantDestroy = true;
      }

      if (this.state == 1 || this.state == 2) {
         this.x += (short)(this.xTo - this.x >> 2);
         this.y += (short)(this.yTo - this.y >> 2);
