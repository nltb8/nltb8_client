...
public class RunWord {
    ...
    public void paintText(mGraphics g) {
        ...
        int i = 0;
        while (i < this.dlgRunY) {
            this.f.drawString(g, this.dlgChainInfo[i], this.dlgX, dlgTextY + i * GameCanvas.hText, 0, false);
            ++i;
        }
        if (this.dlgRunY < this.dlgChainInfo.length) {
            this.f.drawString(g, this.dlgChainInfo[this.dlgRunY].substring(0, this.dlgRunX), this.dlgX, dlgTextY + this.dlgRunY * (GameCanvas.hText - 5), 0, false);
        }
    }
    ...
    public void paintText(mGraphics g, int archor) {
        ...
        if (archor == 2) {
            int i = 0;
            while (i < this.dlgRunY) {
                this.f.drawString(g, this.dlgChainInfo[i], this.dlgX + this.dlgW / 2, dlgTextY + i * (GameCanvas.hText - 5), 2, false);
                ++i;
            }
            if (this.dlgRunY < this.dlgChainInfo.length) {
                this.f.drawString(g, this.dlgChainInfo[this.dlgRunY].substring(0, this.dlgRunX), this.dlgX + this.dlgW / 2, dlgTextY + this.dlgRunY * (GameCanvas.hText - 5), 2, false);
            }
        } else {
            int i = 0;
            while (i < this.dlgRunY) {
                this.f.drawString(g, this.dlgChainInfo[i], this.dlgX, dlgTextY + i * (GameCanvas.hText - 5), 0, false);
                ++i;
            }
            if (this.dlgRunY < this.dlgChainInfo.length) {
                this.f.drawString(g, this.dlgChainInfo[this.dlgRunY].substring(0, this.dlgRunX), this.dlgX, dlgTextY + this.dlgRunY * (GameCanvas.hText - 5), 0, false);
