         ...
         for(i = 1; i < this.entries; ++i) {
            ...
            if (_this > _last) {
               for(int j = _last; j < _this; ++j) {
                  opb.write(i - quantvals, Util.ilog(this.entries - quantvals));
                  quantvals = i;
               }
            }
         }
            ...
            case 0:
               ...
               if (opb.read(1) != 0) {
                  ...
                  while(true) {
                     ...
                     if (opb.read(1) != 0) {
                        quantvals = opb.read(5);
                        if (quantvals == -1) {
                           this.clear();
                           return -1;
                        }

                        this.lengthlist[i] = quantvals + 1;
                     } else {
                        this.lengthlist[i] = 0;
                     }
      ...
      if (this.maptype != 1 && this.maptype != 2) {
         ...
         case 2:
            for(j = 0; j < this.entries; ++j) {
               ...
               for(indexdiv = 0; indexdiv < this.dim; ++indexdiv) {
                  float val = (float)this.quantlist[j * this.dim + indexdiv];
                  val = Math.abs(val) * delta + mindel + last;
                  if (this.q_sequencep != 0) {
                     last = val;
                  }

                  r[j * this.dim + indexdiv] = val;
               }
            }
         }
