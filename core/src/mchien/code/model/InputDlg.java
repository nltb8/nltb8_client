      ...
      if (!this.isTexBoxSV) {
         ...
         if (this.scroll.selectedItem != -1 && this.iscroll) {
            ...
            if (GameCanvas.isPointerClick[0] && GameCanvas.isPointer(this.x, this.y + 30, this.w, this.h - 60, 0)) {
               ...
               this.mTFInput[this.scroll.selectedItem].setFocus(true);
         ...
         if (this.iscroll || !GameCanvas.isTouch) {
            for(i = 0; i < this.mTFInput.length; ++i) {
               ...
               if(i != this.index && this.mTFInput[i].isFocus)
                  this.mTFInput[i].setFocus(false);
//               if (i == this.index) {
//                  if(Gdx.app.getType() == Application.ApplicationType.Desktop && !this.mTFInput[i].isFocus)
//                     this.mTFInput[i].setFocus(true);
//                  this.mTFInput[i].update();
//               } else if(this.mTFInput[i].isFocus) {
//                  this.mTFInput[i].setFocus(false);
//               }
            }
         }
      ...
      if (!this.isTexBoxSV) {
         super.update();
         if(GameCanvas.isTouch&& GameCanvas.isPointerClick[0]&& !this.tfInput.isFocus){
            this.tfInput.title=title;
            this.tfInput.showInput();
            GameCanvas.isPointerClick[0] = false;
         }
