    ...
    public void switchToMe(ScreenTeam lastSCR) {
        ...
        if (GameCanvas.isTouch) {
            ...
            if (!GameCanvas.isTouch) {
                ...
                if ((xx = GameCanvas.w / 2 - (w = size * (colum + 1) + 10) / 2) < 2) {
                    xx = 2;
                }
            } else {
                size = 26;
        ...
        while (i < MainMenu.ListQuest.length) {
            ...
            while (j < MainMenu.ListQuest[i].size()) {
                ...
                if (q != null && (q.status == 1 || q.status == 2 || q.status == 4) && q.idMap != lastid) {
